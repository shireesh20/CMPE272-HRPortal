...
app = Flask(__name__)
app.config.from_pyfile('config.py')  # Configuration settings in a separate file
...
def get_employee_list_data(request_args):
    ...
    employee_query = build_employee_query(only_managers, sort_details, search_query)
...
def view_manager_employees():
    ...
    paginated_employees = fetch_paginated_employees(department_number, employee_search_criteria)
...
def fetch_employee_information(emp_no):
    latest_title_subquery, latest_salary_subquery = build_latest_title_salary_subqueries(emp_no)
...
def update_employee_department(employee, new_dept_no):
    if new_dept_no:
        current_dept_emp = Dept_Emp.query.filter_by(emp_no=employee.emp_no).first()
        if current_dept_emp:
            current_dept_emp.dept_no = new_dept_no
        else:
            db.session.add(Dept_Emp(emp_no=employee.emp_no, dept_no=new_dept_no))
...
def update_title(emp_no, data):
    new_title, from_date, to_date = data.get('title'), data.get('from_date'), data.get('to_date')
    if new_title and from_date and to_date:
        ...
        if existing_title:
            ...
            db.session.add(Title(emp_no=emp_no, title=new_title, from_date=from_date, to_date=to_date))

def update_salary(emp_no, data):
    new_salary, from_date, to_date = data.get('new_salary'), data.get('from_date'), data.get('to_date')
    if new_salary and from_date and to_date:
        existing_salary = Salary.query.filter_by(emp_no=emp_no, from_date=from_date, to_date=to_date).first()
        if existing_salary:
            existing_salary.salary = new_salary
        else:
            db.session.add(Salary(emp_no=emp_no, salary=new_salary, from_date=from_date, to_date=to_date))
...
def add_new_task(task_info):
    try:
        deadline = parse_date(task_info.get('deadline'))  # Handle None case in parse_date function
...
def create_unique_department_number(length=3):
    ...
    while True:
        dept_no = 'd' + ''.join(random.choices(string.ascii_uppercase + string.digits, k=length))
...
def add_new_department():
    ...
    assign_manager_to_department(department_data.get('empNo'), new_department.dept_no)
...
def prepare_new_department(data):
    ...
    new_department = Department(dept_no=unique_dept_no, dept_name=data.get('deptName'))
